...
const unsigned long rttyInterval   = 800;//chaged from 1000
const unsigned long rttyOnTime     = 500;
...
void logEvent(const String &evt) {
  unsigned long elapsed = (millis() - startTime) / 600;//changed from 1000
...
void readAndLog() {
  ...
  // Solenoid control
  if (altitude >= 14.5 && !solenoidFired) { //switches solonoid on 
    solenoidFired = true;
    digitalWrite(SOLENOID_PIN, HIGH);
    logEvent("SOLENOID ON");
  }
  if (altitude < 14.0 && solenoidFired && !solenoidReset) {
    solenoidReset = true;
